...
* 采用双线性插值填充新图
*/
void RotateOP::biliinear_interpolation(CImg<unsigned char>& outImg, 
...
* 采用最邻近插值填充新图
*/
void RotateOP::nearest_interpolation(CImg<unsigned char>& outImg, 
	    const CImg<unsigned char>& srcImg, double theta) {
	int newWidth = outImg.width(), newHeight = outImg.height();
	int srcWidth = srcImg.width(), srcHeight = srcImg.height();
	int halfW = newWidth / 2;
...
CImg<unsigned char> RotateOP::rotate_nearest(const CImg<unsigned char>& srcImg, double theta) {
    ...
    // 开始填充新图片的灰度值
    nearest_interpolation(newImg, srcImg, theta);

    return newImg;	
}

// 旋转图片，旋转角为theta，并采用双线性插值减少锯齿
CImg<unsigned char> RotateOP::rotate_biliinear(const CImg<unsigned char>& srcImg, double theta) {
	int width = srcImg.width();
    int height = srcImg.height();
    // 原图的四个顶点坐标，这里以图片中心为坐标原点
    Position lt(0-width/2, 0+height/2), lb(0-width/2, 0-height/2), 
